...
BACKUP_FILE="$BACKUP_DIR/smartscreen_$TIMESTAMP.dump"
...
if command -v aws &> /dev/null; then
  ...
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Uploaded to s3://$S3_BUCKET/$S3_PREFIX/smartscreen_$TIMESTAMP.dump"
else
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] AWS CLI not found — skipping S3 upload"
fi

find "$BACKUP_DIR" -name "smartscreen_*.dump" -mtime +"$RETENTION_DAYS" -delete
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Cleaned up local backups older than $RETENTION_DAYS days"

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Backup complete: smartscreen_$TIMESTAMP.dump"
