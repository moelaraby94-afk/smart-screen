...
docker exec smart-screen-db-1 psql -U smartscreen -d smartscreen -c "SELECT migration_name, finished_at, rolled_back_at FROM _prisma_migrations WHERE migration_name LIKE '%20260720130000%';"
...
cat apps/backend/prisma/migrations/20260720130000_add_security_analytics_holidays_telemetry_ota/migration.sql | docker exec -i smart-screen-db-1 psql -U smartscreen -d smartscreen -v ON_ERROR_STOP=1

echo "=== Verify tables created ==="
docker exec smart-screen-db-1 psql -U smartscreen -d smartscreen -c "SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename IN ('SecurityEventLog','ProofOfPlay','Holiday','CommandAck','CrashReport','PlayerOtaUpdate') ORDER BY tablename;"

echo "=== Mark migration as applied properly ==="
docker exec smart-screen-db-1 psql -U smartscreen -d smartscreen -c "INSERT INTO _prisma_migrations (id, migration_name, migration, finished_at, applied_steps_count) VALUES (encode(gen_random_bytes(16), 'hex'), '20260720130000_add_security_analytics_holidays_telemetry_ota', '\x', NOW(), 1);"
