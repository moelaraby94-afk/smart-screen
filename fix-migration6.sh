...
docker exec cloud-screen-db-1 psql -U cloudsignage -d cloudsignage -c "SELECT migration_name, finished_at, rolled_back_at FROM _prisma_migrations WHERE migration_name LIKE '%2026072013%' OR migration_name LIKE '%2026072012%';"

echo "=== Delete ALL failed migration records ==="
docker exec cloud-screen-db-1 psql -U cloudsignage -d cloudsignage -c "DELETE FROM _prisma_migrations WHERE finished_at IS NULL;"
